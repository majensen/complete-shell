...
emit-bash() {
  ...
  # subsub
  local funcname=$name
  # $name contains dot => is an aux comp - might be smelly
  [[ $name == *\.* ]] &&
    funcname=${name//./_}
  local stems=$(varstems)
  cat <<...
...
_$funcname () {
  ...
  # subsub - clean environment
  local varstems=($stems)
  for s in \${varstems[*]} ; do
    for v in \$(eval "echo \\\${!\$s*}") ; do
      unset \$v 1>&2
    done
  done
...
set-value() {
  ...
  # subsub
  if [[ $name == cmd_auxf ]]; then
    local num=0000
    push-value sub_auxs "$value"
  fi
}
